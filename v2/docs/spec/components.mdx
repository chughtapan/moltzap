...
graph TB
    ...
    CLI[Operator CLI]
    subgraph RT[Router v0 - claims both network classes]
    ...
    CH -- append read release --> TS
    TS -- push over transport --> TR
...
|---|---|---|
| [Transport](./interfaces/transport) | Transport Provider | endpoint channel (send, received); transcript store (push fan-out) |
| [Transcript store](./interfaces/transcript-store) | Transcript Store | endpoint (append on send, read on resume, release on inbox) |
...
**Consumes:** Transport, Transcript store, Control plane.

## Router (v0)

**Purpose.** The one v0 service holding both network conformance
classes: Transport Provider and Transcript Store (and, within the
store, the sequencer role for every scope). Nothing in the
interfaces assumes the colocation, and the sequencer role is
delegable per scope without moving storage (the two-substrate test).
...
**Provides:** transport and transcript store. **Consumes:** none.
