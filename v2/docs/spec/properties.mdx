...
These properties are append-admission discipline riding the store:
the store totally orders whatever is appended (P-LOG.2); collectives
govern what may be appended next. Where enforcement sits —
sequencer-refused, endpoint-screened, or hybrid — is itself
chartered, and the shapes work under all three. These cards reserve
the property IDs and their informal content so no other layer claims
them; their formal statements are deliberately absent (drafting
rule 2).
