...
- `accept(d)` — a Mailbox Provider durably accepts datagram `d`
...
*Log*, *stream*, and *ledger* name the same ordered sequence at
different altitudes (the abstract object, its conversation view, the
interface serving it).
...
- **Reading.** Nothing is pushed that could not be re-fetched; which
  component holds it is not fixed here (see the
  [mailbox](./interfaces/mailbox) decision).
- **Target.** Mailbox Provider; Sequencer (P-LOG.1 supplies it for
  sequenced entries).
...
- **Target.** Mailbox Provider (drain side); Endpoint (R-EP.4
  resume, R-EP.5 dedup).
...
- **Target.** Transport Provider, Sequencer, Record Store, Mailbox
  Provider.
...
Generic over scopes; the conversation stream and the mailbox inbox
are both deployments ([ledger](./interfaces/ledger)).
...
- **Target.** Sequencer; Endpoint (R-EP.2 checks it).
...
- **Target.** Sequencer; Control Plane (create commits the initial
  entry).
...
- **Statement.** `membership(c, p)` is a pure function of the log
  ...
  same value.
- **Target.** Endpoint (computes it); Sequencer (P-LOG.2 makes it
  well-defined).
...
- **Target.** Mailbox Provider / Transport Provider (fan-out);
...
- **Reading.** A record store that tampers, reorders, or omits is
  auditable by its own contents (P-LOG.2..5).
