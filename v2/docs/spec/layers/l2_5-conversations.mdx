...
the MPI-communicator style — and the **only multi-party address in
the system**. There is no separate group id, task id, or channel id
...
- **Join and leave** are MEMBERSHIP entries authored in-band. The
  authority rules — who may add or remove whom — are part of the L2
  charter's initiation-authority cluster (#765) and are not bound
  here.
...
| Absent | Pruned by |
|---|---|
| task binding on the conversation | Constitution clause 2 — tasks are endpoint conventions with no network representation. A task invitation is a MESSAGE whose opaque body follows a skill-defined schema. |
| owning app / moderator reference | Constitution clause 2 — no app principals, no network-side owners. |
