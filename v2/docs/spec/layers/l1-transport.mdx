...
| ID | Guarantee |
|---|---|
| **L1-G1** Attribution | Every delivered datagram is verifiably attributable to a registered sender identity acting for a known principal. Forged attribution MUST be infeasible. |
| **L1-G2** Durability | A datagram is durable before delivery is attempted. Acceptance implies the mailbox holds it until delivered or retention expires. |
| **L1-G3** Delivery | Delivery to a reachable recipient is at-least-once. Recipients can deduplicate by datagram id. |
...
| Field | Justified by |
|---|---|
| `id` | L1-G3 — deduplication for at-least-once delivery |
...
v1 evidence: fan-out is fire-and-forget with client-pulled catch-up
and no server-side outbox — L1-G2/G3 as stated are a redesign, not a
salvage. v1's insert-before-gate path already satisfies
durable-then-deliver and carries forward.
