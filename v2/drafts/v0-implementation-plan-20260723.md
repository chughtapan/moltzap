...
> verifiers; 18 agents, all completed). **Read the verification outcomes
> first** — the verifiers found real amendments this draft does not yet
> fold in:
>
> **Spec-consistency (Appendix A):** A1 — the §5 dissolution checklist's
> "deliberately never built" header over-forecloses charter/register-open
> items (presence, positive acks, ARCHIVE, escrow, decline): those are
> *deferred, charter-owned*, not never-built; the list must split into
> "never built by decision" vs "absent from v0, owned by an open question."
> A2 — the durable recovery cursor is pre-committed ahead of its own spec
> gate. B1 — PROTOCOL_VERSION's home/source needs restating per the
> carriage record. D1–D3 — citation fixes.
>
> **Completeness (Appendix B):** G1 — the CLI has a skeleton and no
> implementation slices (cli.md's acceptance criterion maps to nothing);
> G2 — the "two harness runtimes interoperate" criterion has no second
> runtime anywhere; G3 — interim authentication on the WS data surface is
> undefined (the signature profile is an HTTP profile); G4 — turn-claim
> carriage has no owning slice; G5 — no slice composes the runnable v2
> server; G6 — endpoint-side conversation initiation (id minting, START
> emission) is unassigned; G7 — docs generation excludes v2 packages;
> G8–G12 — smaller unmapped criteria and mechanics.
>
> None of these invalidate the structure; they are the next editing pass,
> to be folded before the plan is approved and any issue is opened.
...
**The v0 slice.** One deployable vertical: an operator-gated identity registry that mints opaque branded agent ids and issues X.509 Ed25519 cards, with every request on either plane individually signed under the interim RFC 9421 profile (300 s freshness, no sessions, no bearer secrets); a control plane serving exactly the five surviving op families — identity register, directory-read-as-cards, conversation list, transcript read, health — on an interim JSON-RPC-over-HTTP wire that pushes nothing; a transcript store owning one commit-time, gap-free total order per conversation, durable-then-deliver, immutable once committed, with conversation lifecycle riding in-band as L3 entries (START, member-add, leave) and the conversation index derived from them — no control-plane create op exists; a data plane admitting MULTICAST frames content-blind under a per-conversation PCC turn instrument (off-wire, TTL-only expiry), fanning out one-way byte-exact delivery as an optimization over the store; an endpoint channel owning framing/attribution, the durable recovery cursor, L5 gate mounts, and endpoint-resident contacts, with one harness plugin as a pure consumer; a testbed data-plane implementation of the same interface with envelope-level observation and bounded fault injection; and a doc-cited conformance corpus that proves the acceptance criteria, the implementation-swap equivalence gate (data-plane invariant 11), and the two-consumer falsification (bench + arena as pure consumers).
...
| W3 | Control plane: interim JSON-RPC op surface (five families) | control plane |
...
| W6 | Endpoint channel + L5: agnostic core, contacts, gates, recovery cursor, one harness plugin | L1/L2/L3-consumption/L5 / endpoint |
...
4. **Protocol-version value vs. publish exclusion.** All v2 packages are `private: true` (W1); the protocol-version-carriage record makes the v2 protocol package's *published* CalVer the wire version. Interim: `PROTOCOL_VERSION` reads the package's manifest version; enabling publish for the protocol package is a named, future, decision-backed change owned by W3 (`20260723-protocol-version-carriage.md`). Neither enabled nor foreclosed here.
...
Notes: W1's layout doc (W1.S1) is itself spec work and precedes the gate; every other W1 slice and all implementation slices wait on the gate or an explicit maintainer call to open scaffolding early. W4 blocks W5, never the reverse. W8 is downstream of everything and is the final gate. The optional #788 repair (W5.S8) is main-track v1 work on `packages/*` and sits outside this graph.
...
- **∥** W3.S7 (M) control-plane conformance ∥ W3.S8 (S) descriptor-driven doc generation ∥ W2.S9 (M) L1 acceptance pass ∥ W8.S6 (M) membership/lifecycle suite → W8.S7 (M) data-plane delivery suite.
...
- **W6.S12 (L)** harness plugin #1 as pure consumer ∥ **W6.S13 (M)** channel conformance properties.
...
- **W8.S11 (L)** two-consumer falsification harness — the final gate: bench-shaped and arena-shaped consumers over published interfaces only, two runtimes in one conversation, no-internal-reach oracle. A failure here is an interface gap filed against the owning workstream, never patched in the harness.
...
**Deliverables.** Component-to-package map landed as spec text (fills `docs/architecture/components.md`'s deferred section + `v2/AGENTS.md → Structure`, ordinary main PR): cross-cutting `v2/wire` (branded ids, PROTOCOL_VERSION + segment comparator, strict-decode machinery, wire-error base, opaque list cursor), `v2/identity`, `v2/conformance` (W8-owned); control plane `v2/protocol` (interim-labeled), `v2/server`; data plane `v2/plane` (interface seam + production v0 implementation), `v2/testbed-plane` (reserved slot only, pending maintainer call); endpoint `v2/channel`, `v2/cli`, harness-plugin slots (reserved, created by W6 after the adapter-API spec item). Interim non-`@moltzap/` npm scope; `private: true` everywhere. Workspace bring-up via one canary package proving `pnpm build/lint/test` cover v2 unchanged. Compile-clean skeletons for approved packages (closed export maps, empty barrels, owner-workstream READMEs — no implementation code). Hardened `scripts/check-architecture-boundaries.js`: manifest-level v1-dep check, reverse packages/*→v2 check, private-flag guard, export-map assertions, violation-fixture self-tests under `pnpm lint`. CI verified, not invented (`ci.yml` needs no edit; `conformance.yml` untouched until a real suite). Publish exclusion preserved, not decided. New-package template/checklist in `v2/AGENTS.md`.

**Interface sketches.** Package-map entry (name + component + layer(s) + duty + interim/permanent + owner) as the admission contract for every future v2 package. `v2/wire`: branded AgentId/ConversationId, PROTOCOL_VERSION + exact-match segment comparator (missing segments compare as zero), strict-decode guard machinery (normativity register-open), tagged wire-error base, opaque bounded-limit list cursor. Boundary-script contracts: v2-src-no-v1-imports, v2-manifest-no-v1-deps, packages-no-v2-imports, v2-private-until-decided, v2-export-map-closed — each with a fixture that must fail lint.

**Acceptance (cited).** Layout lands as spec text through an ordinary main PR (`20260721-v2-lives-top-level.md`: layout deferred to the spec; `20260722-spec-lives-on-main.md`). Every repo-touching slice waits on the Path-item-4 gate (`v2/VISION.md`). Boundary violations fail mechanically with committed fixtures (`20260721-v2-lives-top-level.md`; `v2/AGENTS.md → Zero v1 imports`; `v2/VISION.md → Provenance`). Every package annotated with exactly one component (`docs/architecture/components.md`) and its layer(s); L4/L6–L8 recorded as deliberate absences (`20260723-eight-layer-stack.md`). Protocol package labeled interim with OpenAPI named as retirement path (`20260722-control-plane-encoding.md`). No production package depends on the testbed-plane slot (data-plane.md invariant 11; `20260723-eval-plane-is-testbed.md`). `publish.yml` allowlist untouched; v2 publish enablement recorded as a future decision (`v2/VISION.md` clause 14; `20260723-protocol-version-carriage.md`). No vacuous-green conformance CI (`v2/inputs/debt-inventory-20260718.md`).
...
**Deliverables.** The v2 conformance framework (re-implemented from the v1 pattern; W8-owned per resolution 1): typed never-throw property registry, consumer-supplied subject factory, pinned-seed replay artifacts, explicit allowed-gap policy — plus two new disciplines: mandatory per-property doc citation (doc + criterion) and a machine-enforced spec/pin partition (normative spec-conformance vs. non-normative implementation pins that may die with the interim wire). Static doc gates in CI: catalog closure over control-plane.md's dissolution table; dissolution-table totality over data-plane.md; no-per-message-principal-verdict spec-set check. Control-plane store suite; identity suite parameterized by attribution binding; data-plane v0 suite; membership/lifecycle suite. Adversity tier over the five tolerated faults + scripted semantic faults, every delivery invariant re-run under each. Implementation-swap equivalence gate (empty outcome diff; one job per resolution 2). Coverage ratchet (every descriptor ≥ 1 property or explicit allowed gap — closing v1's vacuous-green and zero-coverage decay) + machine-checked traceability from the four paper-required constraints (text cited from charter #765, never restated) to properties. Two-consumer falsification harness as the final gate: bench-shaped consumer (experiment observed end-to-end through transcript reads, no database tailing) and arena-shaped consumer (role-scoped visibility from membership + member-scoped reads), two runtimes in one conversation, no-internal-reach oracle (import boundary + published-surface audit).
...
- No task domain: no task ops, no task_id column, no L4 package (✓ inv. 10 property; W1 map).
...
- No bearer secrets, ever; registration returns no secret; the plane stores and returns public material only (✓ no-secret sweep).

**Control plane (`20260723-lifecycle-rides-l3.md`; control-plane.md).**
- No lifecycle op of any kind — create, update, archive, membership, contacts, presence, session — not even interim (✓ catalog-closure check: five families, nothing from *dies*).
...
- No refusal taxonomy: one non-normative interim Refusal; no numeric code registry.

**Store (control-plane.md guarantees 6–8; W4).**
...
- No lease vocabulary on any store surface; no witness-shaped schema column; no retention/horizon machinery.

**Data plane (data-plane.md; #765).**
...
- No presence ops, delivery-status semantics, or positive-ack machinery; push outcomes are content-free telemetry only.
- No ARCHIVE entry type; no escrow/acceptance-quorum semantics; no first-class decline op — refused invitations are agent-local disregard.
- No ops beyond MULTICAST; no entry types beyond START/member-add/leave.

**Identity (identity.md; `20260723-interim-signature-profile.md`).**
- No rotation RPC; no revocation op or status machinery (v0's revocation observable: the registry ceasing to vouch at next resolve); no frame-signing code (interim binding); no peer card custody path; no invite-code shared-secret gate; no absent card fields revived (endpoints, capability flags, skills catalog, transport credential).
...
- No shared limit vocabulary or skill-distributable firewall DSL; no network-wide default posture.

**Workspace and CI (W1).**
- No `@moltzap/*` import or manifest dependency under `v2/*`; nothing under `packages/*` imports v2 (✓ fixtures).
...
| R6 | Adapter API spec item slips | Harness plugin packages and W6.S12, W8.S11 blocked | W6.S1 is M0 work; W6.S3–S11 need only the agnostic core, not the plugin SPI's final shape |
...
Ground truth read: `docs/decisions/README.md` + all nine 2026-07-22/23 records, `v2/VISION.md`, `v2/AGENTS.md`, `docs/spec/{control-plane,data-plane,identity,cli,enforcement}.md`, `docs/spec/endpoints/{contacts,channels,screening,tasks}.md`, `docs/architecture/components.md`, plus the operational surfaces (`.github/workflows/{ci,conformance,publish}.yml`, `scripts/check-architecture-boundaries.js`, `pnpm-workspace.yaml`, root `package.json` docs scripts).
