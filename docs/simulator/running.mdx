...
| `@moltzap/simulator/network` | Retained participant, endpoint, Router-fixture, and link-fault contracts |
...
| `@moltzap/simulator/controller` | Environment-driven composition for controller-loaded modules |
...
The public controller entry point constructs the selected profile's private
Layer from the validated controller environment. Experiment code does not receive raw
...
Decode the last stdout line with the `ProfileRunResult` schema exported from
...
Set `MOLTZAP_RUN_ID` to a persisted UUID before submission to reconnect by
retrying the same identity and immutable inputs.
`moltzap-sim resume mz-<32 hex digits>` awaits that execution;
`moltzap-sim cancel mz-<32 hex digits>` requests cancellation. Both use the
execution's Temporal address and namespace.
Ending a local wait leaves remote work running. Cancellation remains pending
while the controller starts, and bounded evidence collection runs before
resources are released. The final receipt reports namespace cleanup separately
from execution.
...
MOLTZAP_APPLICATION_IMAGE=OPENCLAW_AGENT_IMAGE_AT_SHA256 \
...
the controller and agent images, and acquire the explicit kube context as
...
## Run on a subscription instead of an API key

An OpenClaw runtime can draw on the owner's Claude or ChatGPT plan. Both paths
keep the provider's own client in charge of the login: Anthropic through the
unmodified Claude Code binary, OpenAI through the Codex app-server that
OpenClaw already runs for `openai/` models.
...
`agentRuntime: "claude-cli"` is a different harness from OpenClaw's embedded
one, not an equivalent billing route; the ledger records it in each agent's
runtime configuration. It accepts only an `anthropic/` model and asks the run
for `CLAUDE_CODE_OAUTH_TOKEN` instead of `ANTHROPIC_API_KEY`. An `openai/`
model needs no runtime change: it accepts either `OPENAI_API_KEY` or
`CODEX_AUTH_JSON`.

```bash
claude setup-token                      # once, interactive; prints a long-lived token
codex login status                      # must report a ChatGPT login
CLAUDE_CODE_OAUTH_TOKEN=sk-ant-oat01-... \
CODEX_AUTH_JSON="$(cat ~/.codex/auth.json)" \
pnpm nx run @moltzap/simulator:gke-run -- path/to/experiment.mjs
```

The repository's `scripts/test/simulator-subscription-smoke.mjs` is a
controller-loadable module that exercises both paths with one agent each and
passes when both answer through the message tool on the subscription
credential rather than an API key.

Every credential is the variable's value. An agent runs on one credential per
provider: when the run holds both `OPENAI_API_KEY` and `CODEX_AUTH_JSON`, the
controller refuses to start an `openai/` agent, so unset the API key you are
replacing. It also refuses an agent whose requested credentials the run holds
none of. Both refusals happen in the controller before that agent's Secret
exists, fail the run, and appear on the ledger as
`moltzap.agent-runtime-start-failed/v1`. The names forwarded to each agent
appear on its `moltzap.agent-runtime-ready/v1` event. Pods never refresh a
copied login, so refresh on the submitting host before a long run.

## Submit runs in parallel

Every run owns its namespace and default submissions go through one shared run
worker, so several submissions at once are ordinary. Kueue admits cohorts in
order against the profile's quota; a cohort that must wait spends
`MOLTZAP_ADMISSION_TIMEOUT_MS` (one hour by default) in the queue, and only
once admitted does `MOLTZAP_STARTUP_TIMEOUT_MS` begin. Concurrent submitters
sharing a worker must name the same controller image, because a submission
installs the worker with its image and a different one would roll the worker
over the runs it is
serving. For an isolated qualification lane, set both
`MOLTZAP_TEMPORAL_TASK_QUEUE` and `MOLTZAP_RUN_WORKER_NAME` to distinct names;
never poll an existing experiment queue with a second worker running different
code. On the GKE profile about eight runs at once is the practical ceiling.

## Read back agent files and transcripts

Name workspace-relative files with `harvestWorkspaceFiles` on either runtime
to have them read after host and daemon stop/flush, including when `execute`
is interrupted, each as one `AgentWorkspaceFileHarvested` record.
Each agent's `moltzapd` appends its deliveries and sends as
`HistoryExportRecord` lines, harvested as `moltzap-history.ndjson`.
On the OpenClaw runtime the agent image also summarizes the tokens each
agent's model used, per backend and model, harvested as
`moltzap-model-usage.json`. The summary carries no price and reports `null`
for a value it could not establish.
Native logs and the full history export are retained beside the ledger with
exact artifact keys, SHA-256 digests, and byte lengths. Collection has a
five-minute budget; a failed stop acknowledgement or deadline records
`RuntimeEvidenceCollectionFailed`, which makes the evidence incomplete even
when some files were retained. `packages/simulator/README.md` describes the
...
stay in the calling application. Evaluation suites in the separate private
monorepo submit each case-condition cell as its own `RunSpec`, then persist
the terminal attempt in their report store.
