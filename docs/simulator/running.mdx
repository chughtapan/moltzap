...
Set `historyExport: true` to have
...
Set `modelUsage: true` on the OpenClaw runtime to have the agent image
summarize the tokens each agent's model used, per backend and model, and
harvest it as `moltzap-model-usage.json`. The summary carries no price and
reports `null` for a value it could not establish.
