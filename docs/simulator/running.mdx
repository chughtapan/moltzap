...
> governs new work. The entry points and host-run examples below describe the
> pre-cutover implementation. New execution work uses schema-bound `RunSpec`
> modules and the single Kubernetes `Run.execute` path.
