...
conversation, and message identities. Its transcript projector joins that
exact reference to `ConversationOpened`, `EndpointMessageSent`, and
`EndpointMessageReceived`. This preserves canonical network facts while
making customer topology and selection policy explicit and typed.
