...
export const runSpec = RunSpec.define({
  ...
  execute: ({ agents, events, ledger }) =>
    Effect.gen(function* () {
      yield* events.emit(
        ConsensusReached.make({
          proposal: "initial-proposal",
          supporters: [agents.alice.agentName],
        }),
      );
...
| Field | Meaning |
|---|---|
| `agentName` | Roster-owned identity of the autonomous participant |
...
`execute` receives three run-scoped capabilities:
