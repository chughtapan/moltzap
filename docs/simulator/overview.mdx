...
7. While every application is still up, the controller reads each agent's
   declared workspace files and, when enabled, its daemon's history export
   into the ledger.
...
than collected on exit. `harvestWorkspaceFiles` on either runtime names
