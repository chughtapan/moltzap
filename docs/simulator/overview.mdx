...
| `@moltzap/simulator/runtime` | Container runtime descriptors and the shipped OpenClaw and NanoClaw implementations |
...
catalog, its exact roster, the infrastructure Layer supplied by the selected
profile, and the customer Effect:
...
- `RunInfrastructureFailed` preserves the infrastructure `Cause` and carries a
  completed or incomplete receipt.
