...
## One package, five public entry points
...
| `@moltzap/simulator/controller` | Environment-driven composition for controller-loaded experiment modules |
...
The public controller entry point lets the mounted module select the
controller-owned Layer without exposing Kubernetes, Kueue, Agent Sandbox,
Temporal, or cloud-provider
...
runtime. The separate gateway client starts an OpenClaw `agent` RPC for
...
[OpenClaw integration guide](/integrations/openclaw) for configuration.
...
5. Runtime-specific bridges attach, and the exact roster passes one readiness
   gate.
6. The controller invokes the customer Effect once.
7. The controller requests stop/flush acknowledgements, then reads declared
   workspace files and bounded history exports into the ledger. It
   streams native logs and full history exports into separate retained files
   before releasing the application containers.
...
A completed ledger owns three artifacts:
...
After the program ends, including interruption, the controller stops the host
and daemon while leaving the application entrypoint available for collection.
`harvestWorkspaceFiles` on either runtime names workspace-relative files;
each becomes one `AgentWorkspaceFileHarvested`
record with a `text`, `oversize`, `absent`, or `unreadable` outcome after the
program event. Each agent's own `moltzapd` appends one `HistoryExportRecord`
line per certified delivery and completed send, harvested as
`moltzap-history.ndjson`. On the OpenClaw runtime the agent image also writes a
`moltzap.agent-model-usage/v1` summary of each agent's model tokens once the
host has stopped, harvested as `moltzap-model-usage.json`.

Native logs and full history exports are retained separately from bounded
workspace records. `AgentRuntimeArtifact` records bind their exact keys, byte
lengths, and SHA-256 digests. A failed stop acknowledgement or collection
deadline records `RuntimeEvidenceCollectionFailed`; retained files do not by
themselves establish complete evidence. The fabric still carries opaque packets.
