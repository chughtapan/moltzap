...
Experiment code normally imports the root entry point and `/runtime`.
...
const partitioned = Effect.scoped(
  Effect.gen(function* () {
    ...
    yield* links.shape(
      agents.alice.agent,
      agents.carol.agent,
      linkPolicy.dropAll("lossy uplink"),
      "uplink loss",
    );
...
### Policy applies at the receiver
...
| `effectRuntime` roster agents | Yes | The runtime applies the kernel's `interceptInbound` stage to its own subscription |
| `openClawRuntime` / `nanoclawRuntime` roster agents | No | The agent receives in its own process and the kernel has no control channel into it |

Naming a receiver the kernel cannot reach fails instead of silently shaping
nothing: `LinkController` verbs return a `NetworkFailure` when `to` is not an
in-process receiver. A custom `defineRuntime` implementation is shapeable
exactly when it acquires the `interceptInbound` stage offered in its runtime
input and applies it to the stream it hands its agent.
...
endpoint receiver, and an agent receiver simply never acts on it. Read the
pair of events, never either one alone.
