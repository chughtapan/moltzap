...
Every runtime also owns a Schema and sanitized value describing its
definition-time policy, overrides, and defaults in its native shape. This
configuration does not claim acquisition-resolved host facts. The kernel
records a canonical encoded snapshot under the kernel-owned `agents`
provenance key. Customer provenance is composed around that key and cannot
replace agent, runtime, or configuration evidence. Runtime families do not
normalize model or provider fields into a simulator-wide union, and
credentials never enter this configuration.
...
After ledger allocation, the run returns a closed outcome.
...
failure, before such a reference exists, remains a typed failure of the outer
Effect.
