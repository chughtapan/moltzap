...
7. The controller requests stop/flush acknowledgements, then reads declared
   workspace files and optional bounded history exports into the ledger. It
...
harvests that file as `moltzap-history.ndjson`. `modelUsage: true` on the
OpenClaw runtime has the agent image write a `moltzap.agent-model-usage/v1`
summary of each agent's model tokens once the host has stopped, harvested as
