...
superseded-by: 20260729-kubernetes-secrets-bind-agent-slots.md
---
...
This admitted record is retained for its historical decision and is superseded
by [Kubernetes Secrets bind agent slots](20260729-kubernetes-secrets-bind-agent-slots.md).
The current direct-Agent-Sandbox profile replaces Pod-bound projected-token and
TokenReview enrollment with controller-created, per-slot read-only Secrets.
It retains the separation of Kubernetes workload identity from MoltZap L1
identity, controller-derived roster-slot binding, and release of only the
requesting slot's material.
