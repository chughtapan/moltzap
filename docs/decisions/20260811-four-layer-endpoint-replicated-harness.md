...
host-native messaging, one native session, GENESIS/POST certification, durable
addressed delivery, and `void` completion after local certification. The newer
record and current `docs/spec/` chapters own those interfaces. The historical
Decision Outcome below remains source history; the trace table is updated to
show the current disposition and normative owner for every stable row.
...
| G1-DEC-313 | Re-owned — wake-up traffic is non-authoritative; certified endpoint state is truth. | `docs/spec/conversation-history.md` — Action validity and storage durability and Durable host delivery | `L2`, `L3` |
...
| G1-DEC-403 | Replaced — author-scoped idempotency and evidence-independent private hash ancestry replace central append identity and public START identity. | `docs/spec/conversation-history.md` — Closed values and hashes and Proposal ordering and idempotency; `docs/spec/harness/output.md` — Semantic send | `L3`, `MCP` |
| G1-DEC-404 | Retained — a complete certified record and its retained signer evidence are independently verifiable without a live Registry. | `docs/spec/conversation-history.md` — GENESIS and POST and Persistence and compatibility | `L3` |
...
| G1-DEC-407 | Replaced — the closed action-certificate check is unanimous for GENESIS and author-inclusive `q(n)` for POST; action signatures remain distinct from durability votes. | `docs/spec/harness/tasks.md` — GENESIS, POST, and Durability | `L3`, `PROTO` |
...
| G1-DEC-410 | Replaced — one honest member can still veto unanimous GENESIS; ordinary POST safety instead relies on quorum intersection plus each honest endpoint's durable first-candidate lock under the correct non-equivocating Router assumption. | `docs/spec/layer-interfaces.md` — Trust, safety, and progress; `docs/spec/harness/tasks.md` — GENESIS and Candidate selection | `PROTO` |
...
| G1-DEC-413 | Re-owned — identical post, record, signature, vote, and delivery retries are idempotent; changed intent under one author-scoped `PostId` conflicts. | `docs/spec/conversation-history.md` — Proposal ordering and idempotency and Durable host delivery | `L3`, `MCP` |
...
| G1-DEC-415 | Replaced — fixed members automatically perform verified gap-free catch-up, including pending-delivery reconstruction. | `docs/spec/conversation-history.md` — Catch-up and Router restart and Durable host delivery | `L3` |
...
| G1-DEC-507 | Replaced — each honest endpoint durably locks and signs only the first valid gap-free Router-ordered candidate for one predecessor. | `docs/spec/harness/tasks.md` — Candidate selection | `PROTO`, `MCP` |
...
| G1-DEC-514 | Re-owned — unchanged post intent rebases after a competing commit, while catch-up and quorum re-anchor recover feed gaps and Router restarts. | `docs/spec/conversation-history.md` — Proposal ordering and idempotency and Catch-up and Router restart | `PROTO`, `L3` |
| G1-DEC-515 | Replaced — certified records enter history, while post intents, proposal locks, staged record cores, partial evidence, and pending deliveries are durable recovery state. | `docs/spec/conversation-history.md` — Proposal ordering and idempotency, Action validity and storage durability, Durable host delivery, and Persistence and compatibility | `L3` |
| G1-DEC-516 | Re-owned — members disseminate mergeable signature and vote evidence; transport hints never define truth. | `docs/spec/conversation-history.md` — GENESIS and POST and Action validity and storage durability | `PROTO`, `L3` |
...
| G1-DEC-609 | Replaced — visible model output uses the host's native messaging mechanism with an explicit `agent:` or `group:` destination; plain final text is private. | `docs/spec/harness/output.md` — Native host projection | `MCP`, `INT` |
| G1-DEC-610 | Replaced — the host's durable outbox idempotency key produces the author-scoped `PostId` for every addressed send. | `docs/spec/harness/output.md` — Semantic send; `docs/spec/conversation-history.md` — Closed values and hashes | `MCP`, `L3` |
| G1-DEC-611 | Replaced — adapter-only `send_message` carries exactly idempotency key, explicit destination, and content, with no grant or current-chat authority. | `docs/spec/harness/output.md` — MCP adapter projection | `MCP`, `PROTO`, `WIRE` |
| G1-DEC-612 | Retired — the addressed messaging surface exposes no legal-action descriptors or dynamic action tools. | `docs/spec/harness/output.md` — Native host projection; `docs/spec/harness/tasks.md` — Explicitly deferred | `MCP`, `PROTO` |
...
| G1-DEC-619 | Replaced — the endpoint persists one pending delivery atomically with local certification and marks it acknowledged only after native host durable acceptance. | `docs/spec/conversation-history.md` — Durable host delivery; `docs/spec/harness/ingress.md` — Durable acceptance | `MCP`, `L3` |
| G1-DEC-620 | Replaced — crash after native insertion but before acknowledgment replays the byte-identical stable delivery. | `docs/spec/harness/ingress.md` — Durable acceptance | `MCP` |
| G1-DEC-621 | Replaced — absence of a subscriber preserves pending deliveries; acknowledged stable identities are not invoked again. | `docs/spec/conversation-history.md` — Durable host delivery; `docs/spec/harness/ingress.md` — Durable acceptance | `MCP`, `PROTO`, `L3` |
...
| G1-DEC-623 | Replaced — one active subscriber emits local commit order and preserves strict order within each conversation without a daemon-wide model-turn cap. | `docs/spec/conversation-history.md` — Durable host delivery; `docs/spec/harness/ingress.md` — Delivery projection | `MCP` |
| G1-DEC-624 | Replaced — Client injects no cross-conversation presentation; one native host session supplies cross-address context. | `docs/spec/harness/channels.md` — One native session | `MCP`, `INT` |
| G1-DEC-625 | Replaced — host queueing and scheduling remain host-owned, while every visible post uses native messaging with an explicit address. | `docs/spec/harness/channels.md` — Native messaging | `MCP`, `INT` |
...
| G1-DEC-629 | Re-owned — OpenClaw supervision persists without profiles, consumes Client only, and routes all addressed deliveries through its native main session. | `20260728-endpoint-daemon-speaks-modern-mcp.md` — Supersession and Decision Outcome; `docs/spec/harness/channels.md` — One native session | `INT` |
| G1-DEC-630 | Re-owned — NanoClaw retains one persistent agent daemon without profile state and routes all addressed deliveries through `agent-shared`. | `20260728-endpoint-daemon-speaks-modern-mcp.md` — Supersession and Decision Outcome; `docs/spec/harness/channels.md` — One native session | `INT` |
| G1-DEC-631 | Replaced — runtime bridges use native messaging with an explicit address, and plain final model text cannot bypass that path. | `docs/spec/harness/channels.md` — Native messaging | `INT` |
| G1-DEC-632 | Re-owned — identical completed addressed sends cannot duplicate records; the durable host key resumes private recovery state and returns `void`. | `docs/spec/harness/output.md` — Semantic send | `MCP`, `L3` |
| G1-DEC-633 | Replaced — first and later sends use the same author-scoped idempotency law: identical intent resumes and changed intent conflicts. | `docs/spec/harness/output.md` — Semantic send | `MCP`, `L3` |
...
| G1-DEC-638 | Replaced — one native host session supplies cross-address context; Client snapshots and presentation checkpoints are absent. | `docs/spec/harness/channels.md` — One native session | `MCP`, `INT` |
| G1-DEC-639 | Replaced — inbound content and transport acknowledgment are independent from output authority; every visible response is a new explicit addressed send. | `docs/spec/harness/ingress.md` — Durable acceptance; `docs/spec/harness/output.md` — Semantic send | `MCP`, `PROTO` |
...
| G1-DEC-641 | Replaced — the portable model-output surface is one explicit addressed send through host-native messaging and returns `void` after local certification. | `docs/spec/harness/output.md` — Semantic send and Native host projection | `MCP`, `L3`, `PROTO` |
...
| G1-DEC-819 | Resolved — Client supplies no snapshot or presentation checkpoint; one native host session and host scheduling own cross-address context and concurrency. | `docs/spec/harness/channels.md` — One native session and Native messaging | `MCP`, `INT` |
