...
| G1-DEC-609 | Replaced — the adapter accepts only a host-supplied explicit `agent:` or `group:` destination; model tools and final-text interpretation remain stock host behavior. | `docs/spec/harness/output.md` — Stock host projection | `MCP`, `INT` |
...
| G1-DEC-619 | Replaced — the endpoint persists one pending delivery atomically with local certification and marks it acknowledged only after the stock host inbound callback completes successfully; the host owns what persistence that callback represents. | `docs/spec/conversation-history.md` — Durable host delivery; `docs/spec/harness/ingress.md` — Durable acceptance | `MCP`, `L3`, `INT` |
...
| G1-DEC-625 | Replaced — host queueing, scheduling, model-output interpretation, and destination discovery remain host-owned, while every adapter callback carries an explicit address. | `docs/spec/harness/channels.md` — Adapter messaging | `MCP`, `INT` |
...
| G1-DEC-630 | Re-owned — NanoClaw retains one persistent agent daemon without profile state and consumes Client only through its stock channel API; session, inbox, ACL, and sandbox behavior remain host-owned. | `20260728-endpoint-daemon-speaks-modern-mcp.md` — Supersession and Decision Outcome; `docs/spec/harness/channels.md` — Stock host boundary | `INT` |
| G1-DEC-631 | Replaced — runtime adapters validate one explicit address per outbound callback; whether model output creates that callback remains stock host behavior. | `docs/spec/harness/channels.md` — Adapter messaging | `INT` |
