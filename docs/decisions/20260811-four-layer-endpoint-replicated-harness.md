...
| G1-DEC-501 | Replaced — every send names an `@<AgentName>` or `group:@<AgentName>,...` address; Client resolves immutable AgentCards, inserts self for groups, and rejects invalid membership. | `docs/spec/conversation-history.md` — Addresses and membership; `docs/spec/harness/client.md` — Addressed send | `PROTO`, `ID`, `MCP` |
...
| G1-DEC-609 | Replaced — the adapter accepts either the stock current-origin reply callback or a host-supplied explicit `@<AgentName>` or `group:@<AgentName>,...` proactive destination; model tools and final-text interpretation remain stock host behavior. | `docs/spec/harness/output.md` — Stock host projection | `MCP`, `INT` |
...
| G1-DEC-611 | Replaced — adapter-only `send_message` carries exactly an explicit `@<AgentName>` or `group:@<AgentName>,...` destination and content, with no host queue identity, grant, or current-chat authority. | `docs/spec/harness/output.md` — MCP adapter projection | `MCP`, `PROTO`, `WIRE` |
...
| G1-DEC-625 | Replaced — host queueing, scheduling, model-output interpretation, and destination discovery remain host-owned; stock output callbacks resolve to the current inbound address or an explicit proactive address. | `docs/spec/harness/channels.md` — Adapter messaging | `MCP`, `INT` |
...
| G1-DEC-631 | Replaced — runtime adapters bind a reply callback to its current inbound address or validate one explicit proactive address; whether model output creates that callback remains stock host behavior. | `docs/spec/harness/channels.md` — Adapter messaging | `INT` |
...
| G1-DEC-641 | Replaced — the portable model-output surface is one addressed send through stock host callbacks and returns `void` after local certification. | `docs/spec/harness/output.md` — Semantic send and Stock host projection | `MCP`, `L3`, `PROTO` |
...
| 2026-08-31 | Updated addressed-send trace rows to the current `@`-prefixed runtime spelling. The four-layer endpoint-replicated Decision Outcome is unchanged. |
