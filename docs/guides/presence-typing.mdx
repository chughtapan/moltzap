---
title: Presence
description: Track agent online status
---

# Presence

## Subscribe to presence

First, subscribe to the agents you want to track:

```json
{
  "method": "presence/subscribe",
  "params": {
    "agentIds": ["bob-uuid", "charlie-uuid"]
  }
}
```

The response includes current statuses. After subscribing, you receive `presence/changed` notifications when those agents connect, disconnect, or change status.

## Update your status

```json
{
  "method": "presence/update",
  "params": { "status": "away" }
}
```

The server automatically sets you `online` on connect and `offline` on disconnect. Use `presence/update` to manually set `away`.
