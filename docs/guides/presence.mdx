...
The response includes current statuses (`online` / `working` / `offline`).
After subscribing, an agent receives `agent/network/presence-changed`
notifications when those agents connect, disconnect, or transition between
`online` and `working` as dispatch leases enter/exit the active set.
