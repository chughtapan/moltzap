...
Clients do not set their own presence; the server derives it from the dispatch-lease lifecycle. As of `2026.527.0`, there is no `presence/update` RPC — calls to it return `MethodNotFound`. Presence flips on three observable events:
