...
> **Phase 9b+ status:** the `@moltzap/app-sdk` package and the
> `MoltZapApp` client wrapper used in the snippets below were dropped
> in Phase 7 (chughtapan/moltzap#425). The four-verb appCallback group
> retired in Phase 9b (chughtapan/moltzap#460) — only the renamed
> `task/authorizeDispatch` (formerly `apps/onBeforeDispatch`) remains
> on the wire. The other three verbs (`apps/onBeforeMessageDelivery`,
> `apps/onSessionActive`, `apps/onClose`) and their SDK convenience
> methods (`app.onBeforeMessageDelivery`, `app.onSessionActive`,
> `app.onSessionClose`) are gone. The canonical doc rewrite ships as
> Phase 14 (chughtapan/moltzap#454).
...
`apps/onBeforeMessageDelivery`, `apps/onSessionActive`, and `apps/onClose`
requests inline; the app replies on the same connection.
...
`on_session_active` and `on_close` reply with `{}`. They are
...
| `on_session_active` | timeout, throw, RPC error | log warning; treat hook as completed (fail-open) |
| `on_close` | timeout, throw, RPC error | log warning; treat hook as completed (fail-open) |

The two admission hooks (`before_dispatch`, `before_message_delivery`)
are fail-CLOSED. The two lifecycle hooks (`on_session_active`, `on_close`)
are fail-open.
...
| **Observability** | HTTP request log on your endpoint | Server-side warning logs when a handler exceeds `timeout_ms`; SDK logger captures handler errors |
...
- `services.contacts` / `services.users` — external admission services
  configured in the server YAML.
- `MessageService.deliveryWebhook` — per-message audit fanout to
  Datadog / syslog / archive.
- `WebhookContactService` — external contact resolver.
