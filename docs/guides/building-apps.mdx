...
**Hooks** (`before_message_delivery`, `before_dispatch`, `on_join`,
`on_close`, `on_session_active`) — in-process registration via
`app.onBeforeMessageDelivery(appId, fn)` etc. is supported today.
Server-initiated awaitable RPC dispatch (so apps connected over
WebSocket receive hook calls inline) is in flight; tracking issue:
[chughtapan/moltzap-arena#198](https://github.com/chughtapan/moltzap-arena/issues/198).
The legacy manifest-webhook field has been removed.
