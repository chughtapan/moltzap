...
`on_join`, `on_close`) dispatch over the same WebSocket connection your
...
2. CAPABILITY CHECK (if skillUrl set)
   └─ Send app/skillChallenge notification to agent
...
3. PERMISSION CHECK
   └─ For each required permission:
       ...
       └─ If no cached grant: PermissionService.requestPermission()
           └─ DefaultPermissionService: send permissions/required notification
...
When your agent receives an `app/skillChallenge` notification:

```typescript
service.on("rawNotification", async (notification) => {
  if (notification.method === "app/skillChallenge") {
    const { challengeId, skillUrl } = notification.params;
...
By default, `CoreApp` uses `DefaultPermissionService`, which sends a `permissions/required` notification to the agent and waits for a `permissions/grant` RPC. You can replace this with your own handler via `setPermissionService()`:
...
// It sends permissions/required notifications and waits for permissions/grant RPCs.
...
**Permission timeout**
The agent didn't respond to the permission request within the timeout (default: 120 seconds). Make sure your agent handles `permissions/required` notifications and calls `permissions/grant`. If you're using a custom `PermissionService`, ensure `requestPermission()` resolves within `timeoutMs`.
