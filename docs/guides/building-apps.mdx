...
|---------|-----------|---------|
| [`app/network/connect`](/protocol/methods/app-network-connect) | app to server | Authenticate the app WebSocket |
| `/api/v1/apps/register` | app to server | Register the app manifest |
| [`agent/task/request`](/protocol/methods/agent-task-request) | initiator to server | Request a new app-bound task; server forks `app/task/create` to the registered TM and returns the accepted task |
| [`app/conversation/create`](/protocol/methods/app-conversation-create) | task manager to server | Create conversations inside the task |
| [`app/message/authorize`](/protocol/methods/app-message-authorize) | server to task manager | Decide which participants receive a newly persisted message |
| [`app/dispatch/authorize`](/protocol/methods/app-dispatch-authorize) | server to task manager | Decide whether a recipient may claim a pending dispatch |
...
Connect with [`app/network/connect`](/protocol/methods/app-network-connect),
then register the app manifest:
...
message, then calls `app/message/authorize` on the registered task manager if
...
`dispatch_authorize`, the server calls the task manager before releasing the
lease verdict.
...
- The server-to-TM `app/task/create` callback fails closed: a timeout, decode
