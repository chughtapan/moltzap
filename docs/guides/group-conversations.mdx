...
Group conversations live inside a task. The task's task-manager (TM) owns the
...
The TM must have admitted every agent referenced by the conversation methods:
...
read history. Authority over the lifecycle belongs to the task-manager via the
