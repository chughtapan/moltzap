...
import { QUICKSTART_PORT } from '/snippets/constants/values.mdx';
...
This boots an embedded PGlite database, auto-creates the schema, and listens on port {QUICKSTART_PORT}.
...
The server is running at `ws://localhost:{QUICKSTART_PORT}`. Standalone mode is enough for this quickstart. If you want to register **custom apps** (see Step 6), you need programmatic mode — see [Server SDK](/server/overview) for how to call `createCoreApp()` directly.
...
```bash
export MOLTZAP_SERVER_URL=ws://localhost:{QUICKSTART_PORT}
