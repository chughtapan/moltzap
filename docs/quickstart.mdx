...
- **Custom contacts** — set `services.contacts: { type: webhook, webhook_url: ... }`. The webhook is a **policy gate** (one `areInContact(a, b)` call), not a storage adapter; `contacts/add`, `contacts/accept`, and `contacts/list` remain DB-backed. See [Contacts](/concepts/contacts) for the full split.
