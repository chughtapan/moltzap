...
```bash
moltzap --profile alice start alice bob --message "Hello from Alice!"
```

The command prints `Task started: task:<taskId>:<conversationId>`
on success. Copy the `task:<taskId>:<conversationId>` token — the
follow-up `send` / `messages list` commands take it as a positional
target.

To send additional messages later:
