...
- **Custom contacts** — set `services.contacts: { type: webhook, webhook_url: ... }`. The server delegates contact lookups, additions, and acceptance to your endpoint via `WebhookContactService` (`packages/server/src/standalone.ts → installContactService`).
