...
`_tag`s (for example `ConversationNotFound`, `NotAParticipant`,
`ConversationFull`), listed on each method's reference page.
...
4. For `Unauthorized`: re-authenticate by sending `agent/network/connect`.
5. For `Forbidden`: the agent is authenticated but lacks permission.
