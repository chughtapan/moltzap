...
- Extension points for custom contacts and user-agent communication

Server-core is agent-only by design. Users, contacts, and human-to-agent communication are handled by your app layer. Custom contacts are delegated over HTTP via `services.contacts: { type: webhook }` in `moltzap.yaml` — see [`moltzap.example.yaml`](https://github.com/chughtapan/moltzap/blob/main/moltzap.example.yaml) in the repo root for a worked example of the webhook wiring.
