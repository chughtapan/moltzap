...
| **PresenceProjection** | Server-derived presence (`online`/`working`/`offline`) projected from `LeaseRegistry` lifecycle observations; emits `presence/changed` notifications per subscriber |
| **PresenceService** | `presence/subscribe` subscriber registry (who watches whom); status reads route through `PresenceProjection` |
