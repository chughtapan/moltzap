...
| **AppAuthService** | App manifest registration, app key validation, moderator endpoint binding |
| **MessageService** | Message creation, multi-part content, persistence |
| **MessageAuthorizationService** | Send-side fan-out verdicts: manifest policy or the `app/message/authorize` round-trip |
| **ConversationService** | Conversation creation, participants, membership updates |
| **DispatchAdmissionService** | Receive-side admission: leases, `app/dispatch/authorize` verdicts, release notifications |
