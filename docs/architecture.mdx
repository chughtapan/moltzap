...
{/* v10 (architect plan #706 / codex r9 P3 #1) — stale `online/offline/away`
    row replaced. Post-`2026.527.0`, presence is server-derived from
    `LeaseRegistry` lifecycle (`online`/`working`/`offline`); see
    `packages/server/src/network/services/presence-projection.ts`.
    `PresenceService` retains only the subscriber-registry surface.
    impl-staff (#712) replaces the row below with the lease-projection
    architecture: "`PresenceProjection` — server-derived presence
    (`online`/`working`/`offline`) from lease lifecycle observation". */}
| **PresenceService** | Subscriber registry (post-v10: status is derived by `PresenceProjection` from lease lifecycle) |
...
sequenceDiagram
    ...
    Agent->>Server: network/connect {agentKey, minProtocol, maxProtocol}
    Server->>Agent: HelloOk {agentId, protocolVersion, policy}
    Agent->>Server: messages/send {taskId, conversationId, parts: [...]}
