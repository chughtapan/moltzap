...
|---|---|---|
| Registry | A thin directory library on `@peculiar/x509` + Ed25519: `lookup`/`list` serving cards over the plane, with minting a library call the deployment drives out of band (`docs/decisions/20260727-registration-is-out-of-band.md`); every minted card appended to a signed log from day one (the raw material for a verifiable directory later) | A CA — the wrong shape for a key directory; key-transparency proofs; the fact vocabulary beyond the active bit; any plane-side admission op |
| Signer / Verifier | Ed25519 over the message's bytes, card-key custody in the endpoint composition. Recipients and monitors verify from the message and the card alone — no interim concession, no migration to schedule (`docs/decisions/20260726-attribution-binds-to-the-message.md`) | Rotation and revocation (register item 5) |
| Ledger | Append-only per-conversation log on an ordinary relational store; atomic `append`, `read` by offset, membership as a fold over recorded actions; a start message to a fresh id creates the log; the committed chain is hash-linked from day one. The append path admits what it records and is the one that **enforces** the grant — a non-holder's effective write is refused, since endpoint typestate binds only honest endpoints | The chain's audit/verification surface; any completeness judgment; escrow of any kind |
| Transport | The interim WebSocket carriage behind the Transport tag: `send`, `subscribe` resumable from an owned offset; admission on the delivery path (verify, member-or-fresh-START, version) inside the adapter, which checks no grant — a protocol's messages need none. Round-one lock defaults, all implementation not spec: FIFO by arrival per conversation; `begin` waits its turn and refuses when the wait outlives the TTL; the grant is pinned across an in-flight append so expiry never interrupts one; a refused write leaves the grant held (retry in place) and staging does not extend the TTL; after a router restart, in-memory grants are gone and holders re-`begin`; a protocol's messages were never recorded, so there is nothing to re-fold. Carriage is now free: attribution rides the message, so the socket carries bytes and nothing depends on the connection. The remaining recorded deviation is mixed traffic on one surface (`docs/spec/data-plane.md` → Implementation notes) | The target wire (Q10); strict one-way notification cleanup rides #788 |
