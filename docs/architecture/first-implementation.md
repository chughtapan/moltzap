...
| Signer / Verifier | The interim profile: RFC 9421 request signing, Ed25519, card-key custody in the endpoint composition. Recipients inherit the router's admission-time verification; offline per-frame re-verification arrives with the target binding (`docs/spec/identity.md` → Implementation notes) | Per-frame target binding (register item 5); rotation |
