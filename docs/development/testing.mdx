...
`CoreTestServer` handle type. Deeper DB/PGlite helpers remain internal server
test infrastructure under `packages/server/src/test-utils/`; tests inside
`packages/server` import those concrete helper modules directly.
