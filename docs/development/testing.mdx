...
- `MoltZapWsClient` — The production WebSocket client. `waitForNotification(definition, timeoutMs)` and `drainNotifications()` are available on it directly, so tests can await specific notifications without wiring their own callbacks.
