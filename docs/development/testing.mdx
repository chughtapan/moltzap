...
import { registerAgent, MoltZapAgentClient, MoltZapAppClient } from "@moltzap/client";
import {
...
`resetCoreTestDb`, `getCoreApp`, `getCoreDb`, `getBaseUrl`, `getWsUrl`),
`createTestAgent`, `expectRpcFailure`, and the PGlite harness. Client-side
multi-agent scaffolding (`closeAllClients`, group/pair builders) remains
internal test infrastructure under `packages/server/src/test-utils/helpers.ts`;
tests inside `packages/server` import it relatively.
