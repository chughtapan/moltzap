...
### Remaining Server Migration Order

1. Split `AppHost` into app endpoint registry plus domain callback/admission
   services.
2. Move conversation and message services/handlers out of `task/` into their
   own domain folders.
3. Move lease registry and dispatch handlers into `dispatch/`.
4. Move server-side requirement checks beside their domains.
5. Recompose the handler catalog from domain handler modules.
6. Delete the old `app/` folder once empty.
