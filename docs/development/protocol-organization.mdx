...
message are peer domains. Dispatch is message moderation, so it stays under the
message domain. `socket` owns the runtime factories that open agent/app clients
and the server.
...
- `app/conversation/update` belongs in `conversation/conversations.ts`

Conversation and message are peer domains. Dispatch is the message moderation
lifecycle, so its ids, methods, callbacks, notifications, and decisions live
under `message/dispatch.ts`.
...
route. App registration posts an `AppManifest` to `/api/v1/apps/register`, so
`identity/apps/manifest.ts` owns that route's schema.
...
caller joins the conversation it creates and names the `appId` whose policy
moderates it. `app/conversation/update` owns participant add and participant
remove semantics; the server rejects a caller that does not own the
conversation.
...
uses it: `message/authorization.ts` and `dispatch/admission.service.ts`.

`conversation/`, `message/`, and `dispatch/` are peer folders.
...
- app ownership of a conversation -> `conversation/requirements/`
...
1. Identity owns agent and app registration and auth.
2. Network owns connect handlers and outbound notification send.
3. Conversation, message, and dispatch each expose a flat
