...
11. Rename wire methods to the principal-prefixed naming scheme.
12. Regenerate protocol reference docs from the new descriptors.

Those steps are complete for the protocol package. Follow the same order for
future package rebalances: establish aliases and barrels first, then move code,
then update wire names and generated docs together when a compatibility break is
intentional.
...
7. Agent identity handlers/services, app auth, contacts, connect handlers, and
   presence are now under their domain folders and imported through server
   source aliases.
