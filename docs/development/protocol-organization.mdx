...
transport
  -> identity
  -> network
  -> task / conversation / message / dispatch
  -> catalog
  -> socket
  -> testing
```
...
conversation, message, and dispatch are peer domains. `socket` owns the runtime
factories that open agent/app clients and the server.
...
- `app/dispatch/authorize` belongs in `dispatch/callbacks.ts`
- `app/task/create` belongs in `task` because it is a task callback

Task, conversation, message, and dispatch are peer domains. Do not nest
conversation or message under task just because their requirements include
`taskId`.
...
`app/task/update` replaces separate task participant add/remove and task close
commands. Its schema owns validation for which changes are allowed together.
...
`app/conversation/update` replaces separate archive, unarchive, participant
add, and participant remove methods. Requirements still enforce task ownership
...
server-pushed events, not method siblings.
...
Each domain can have a `catalog.ts` for descriptor arrays used by protocol
composition. Catalog files are internal package implementation details and
...
```json
{
  ...
  "./dispatch": "./dist/dispatch/index.js",
...
specifiers. Relative imports are only for the barrel/catalog files that assemble
...
9. Build internal `catalog.ts` composition from domain `catalog.ts` files.
...
`core/` owns server boot and wiring. Files currently named `app/layers.ts`,
`app/server-handlers.ts`, `app/server-handlers-runtime.ts`, `app/tracing.ts`,
and `core-server.ts` belong under `core/`.

`socket/` owns WebSocket connection lifecycle and request-scoped transport
context. Files currently under `transport/` that are about live socket
connections move to `socket/`. Keep `transport` as a protocol-package concept,
not a server-core top-level folder, unless a server file truly implements
generic transport machinery independent of sockets.
...
1. Add `core/`, `socket/`, `http/`, `conversation/`, `message/`, and
   `dispatch/` folders without moving behavior.
2. Add server-core TypeScript `paths`, `tsc-alias` build rewriting, and
   matching source-test alias resolution.
3. Move boot/wiring files out of `app/` into `core/`, rewriting imports to
   absolute barrels as files move.
4. Move socket lifecycle files from `transport/` to `socket/`.
5. Move HTTP files from `app/` to `http/`.
6. Split `AppHost` into app endpoint registry plus domain callback/admission
   services.
7. Move conversation and message services/handlers out of `task/` into their
   own domain folders.
8. Move lease registry and dispatch handlers into `dispatch/`.
9. Move server-side requirement checks beside their domains.
10. Recompose the handler catalog from domain handler modules.
11. Delete the old `app/` and server `transport/` folders once empty.

This migration should follow the protocol reorg. Keep wire names stable until
the protocol descriptor rename step is scheduled.
