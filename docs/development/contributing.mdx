...
The docs site at `docs/` is built with [Mintlify](https://mintlify.com). Protocol reference pages (under `docs/protocol/methods/` and `docs/protocol/notifications/`) are **auto-generated** from the TypeBox schemas and descriptors in `packages/protocol/src/schema/`.
...
- **Protocol method/notification pages** — do NOT edit directly. Change the schema or the generation script (`scripts/generate-protocol-docs.ts`), then run `pnpm docs:generate`
