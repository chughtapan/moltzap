...
The docs site at `docs/` is built with [Mintlify](https://mintlify.com). Protocol reference pages (under `docs/protocol/methods/` and `docs/protocol/notifications/`) are **auto-generated** from the schemas and descriptors in `packages/protocol/src/`, which each domain declares in its own file (`identity/agents/agents.ts`, `network/connect.ts`, `conversation/conversations.ts`, `message/messages.ts`, `message/dispatch.ts`).

### Updating protocol docs after schema changes

When you modify a schema, you need to regenerate the protocol docs:
...
| `pnpm docs:generate` | Regenerate protocol reference MDX from Effect `Schema` definitions |
