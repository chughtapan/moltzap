...
- every code-peer send or receive observation has exactly one
  `RouterMessageCommitted` record for the same task, conversation, and
  message;
...
| `SocialTranscriptItem` | Code-peer observation, direction relative to the target, peer endpoint, task and conversation identities, router-commit evidence identity, and parts |
