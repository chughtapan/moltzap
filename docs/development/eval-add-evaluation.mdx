...
> peer behavior, criteria, and grading remain evaluation-owned code. The
> current `effectRuntime` peer builder is transitional host machinery; the
> Kubernetes runtime packages that policy as a peer-container entrypoint and
> exposes its exact observation gateway through a peer-specific bridge.
...
The current host implementation builds bundled peers as autonomous
`effectRuntime` policies. They send and receive through
`EffectRuntimeContext.client`, so their social traffic traverses the
production protocol and router. The Kubernetes path runs the same policy in
the peer's application container. Its peer-specific bridge exposes an
