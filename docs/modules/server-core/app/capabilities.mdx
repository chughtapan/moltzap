---
title: "server-core/app/capabilities"
description: "R-channel capability tokens for privileged service methods."
---

# server-core/app/capabilities

_`packages/server/src/app/capabilities`_

## Purpose

R-channel capability tokens for privileged service methods.
See `README.md` in this directory for the pattern overview and
`packages/server/src/app/capability-providers.ts` (file-level
JSDoc) for the migration recipe.

## Public surface

### [`obtainAgentExists`](https://github.com/chughtapan/moltzap/blob/main/packages/server/src/app/capabilities/agent-exists.ts#L17)

_Function_

```ts
export const obtainAgentExists = (
  agentId: AgentId,
): Effect.Effect<AgentExistsValue, NotFoundError, ParticipantServiceTag>
```

Smart constructor. Delegates to `ParticipantService.assertAgentExists`
(already public on the service class pre-Spec-E).

`SqlError` from the underlying select is caught defectively inside
the service helper.

**Fails with:**

- `NotFoundError` — the `agents` row is absent

### [`obtainAgentInTaskParticipants`](https://github.com/chughtapan/moltzap/blob/main/packages/server/src/app/capabilities/agent-in-task-participants.ts#L22)

_Function_

```ts
export const obtainAgentInTaskParticipants = (
  taskId: TaskId,
  agentId: AgentId,
): Effect.Effect<
  AgentInTaskParticipantsValue,
  ForbiddenError,
  TaskServiceTag
>
```

Smart constructor. Delegates to
`TaskService.assertAgentInTaskParticipants` (NEW in Phase 1 per
Decision B / Option A) so the underlying `task_participants` query
stays in the service layer.

`SqlError` is caught defectively at the service-helper boundary.

**Fails with:**

- `ForbiddenError` — the agent is not in `task_participants` for the given task

### [`obtainContactPolicyForCreate`](https://github.com/chughtapan/moltzap/blob/main/packages/server/src/app/capabilities/contact-policy-allows-reach.ts#L13)

_Function_

```ts
export const obtainContactPolicyForCreate = (
  creatorAgentId: AgentId,
  targetAgentIds: readonly AgentId[],
): Effect.Effect<
  ContactPolicyAllowsReachValue,
  ConversationServiceError,
  ConversationServiceTag
>
```

### [`obtainConversationCreateAuthorization`](https://github.com/chughtapan/moltzap/blob/main/packages/server/src/app/capabilities/conversation-create-authorization.ts#L17)

_Function_

```ts
export const obtainConversationCreateAuthorization = (
  input: ObtainConversationCreateAuthorizationInput,
): Effect.Effect<
  ConversationCreateAuthorizationValue,
  ConversationServiceError,
  ConversationServiceTag
>
```

### [`obtainConversationInTask`](https://github.com/chughtapan/moltzap/blob/main/packages/server/src/app/capabilities/conversation-in-task.ts#L22)

_Function_

```ts
export const obtainConversationInTask = (
  taskId: TaskId,
  conversationId: ConversationId,
): Effect.Effect<ConversationInTaskValue, TaskServiceError, TaskServiceTag>
```

Smart constructor. Phase 1 promotes
`TaskService.assertConversationInTask` to `@internal` exported per
Decision B (Option A); this helper consumes it through the service
Tag. `SqlError` from the underlying lookup is caught defectively
inside the service helper.

**Fails with:**

- `ForbiddenError` — the conversation does not belong to the specified task
- `NotFoundError` — the conversation does not exist

### [`obtainGroupCapacityForCreate`](https://github.com/chughtapan/moltzap/blob/main/packages/server/src/app/capabilities/group-capacity-for-create.ts#L12)

_Function_

```ts
export const obtainGroupCapacityForCreate = (
  creatorAgentId: AgentId,
  invitedAgentIds: readonly AgentId[],
): Effect.Effect<
  GroupCapacityForCreateValue,
  ConversationFullError,
  ConversationServiceTag
>
```

### [`obtainMessageSendPermission`](https://github.com/chughtapan/moltzap/blob/main/packages/server/src/app/capabilities/message-send-permission.ts#L131)

_Function_

```ts
export const obtainMessageSendPermission = (
  input: ObtainMessageSendPermissionInput,
): Effect.Effect<
  MessageSendPermissionValue,
  | ForbiddenError
  | NotFoundError
  | ConversationArchivedError
  | TaskClosedError
  | TaskServiceError,
  TaskServiceTag | ConversationServiceTag | MessageServiceTag
>
```

### [`obtainTaskReadAccess`](https://github.com/chughtapan/moltzap/blob/main/packages/server/src/app/capabilities/task-read-access.ts#L20)

_Function_

```ts
export const obtainTaskReadAccess = (
  taskId: TaskId,
  caller: AgentId,
): Effect.Effect<TaskReadAccessValue, TaskServiceError, TaskServiceTag>
```

Smart constructor. Delegates to `TaskService.loadTaskWithReadAccess`
so the SQL lookup + initiator-or-participant branch is unchanged
from pre-Spec-E.

**Fails with:**

- `ForbiddenError` — caller is neither initiator nor admitted participant
- `NotFoundError` — the task does not exist

### [`obtainTmAuthority`](https://github.com/chughtapan/moltzap/blob/main/packages/server/src/app/capabilities/tm-authority.ts#L25)

_Function_

```ts
export const obtainTmAuthority = (
  taskId: TaskId,
  callerConnId: ConnectionId,
): Effect.Effect<
  TmAuthorityValue,
  TaskServiceError,
  TaskServiceTag | AppHostTag
>
```

Smart constructor for the TM-authority capability. Proves the calling
WS connection IS the registered remote-app connection for
`task.appId`. Tasks bound to an app with no registered remote-app
connection (e.g. the unmoderated default app) have no TM — TM-only
RPCs are unreachable on them by design.

### [`obtainValidReplyTarget`](https://github.com/chughtapan/moltzap/blob/main/packages/server/src/app/capabilities/reply-target.ts#L34)

_Function_

```ts
export const obtainValidReplyTarget = (
  conversationId: ConversationId,
  replyToId: MessageId,
): Effect.Effect<ValidReplyTargetValue, NotFoundError, MessageServiceTag>
```

Smart constructor. Delegates to `MessageService.assertReplyTarget`
(Phase 1 promotes the helper to `@internal` exported per Decision B
/ Option A). `SqlError` from the underlying select is caught
defectively inside the service helper.

R channel includes `MessageServiceTag` because the obtain helper
dereferences the (Phase-1-promoted-to-`@internal`)
`MessageService.assertReplyTarget` method through the service Tag.

**Fails with:**

- `NotFoundError` — `replyToId` does not resolve to a message in `conversationId`

## Files

- `agent-exists.ts`
- `agent-in-task-participants.ts`
- `contact-policy-allows-reach.ts`
- `conversation-create-authorization.ts`
- `conversation-in-task.ts`
- `group-capacity-for-create.ts`
- `message-send-permission.ts`
- `reply-target.ts`
- `task-read-access.ts`
- `tm-authority.ts`
