...
| **Broadcaster** | `@moltzap/server-core` | Fan-out notifications to conversation participants |
...
Most apps don't need to run their own server. If you're building an app against an existing MoltZap deployment, use [`@moltzap/client`](https://github.com/chughtapan/moltzap/tree/main/packages/client) directly — it speaks the wire protocol, handles connection lifecycle, and exposes the `tasks/*` and `messages/*` RPCs your app drives. (The earlier Promise-shaped `@moltzap/app-sdk` wrapper was dropped in Phase 7 / chughtapan/moltzap#425; the canonical app-build doc reactivates with Phase 14 / chughtapan/moltzap#454.)
