...
| `DuplicateServerRpcHandlerError`<br/>**(Spec F #617 — scheduled for deletion)** | `@moltzap/client`<br/>`runtime/errors.ts → DuplicateServerRpcHandlerError` | Legacy `handleServerRpc()` called twice for same definition. Spec F replaces the dynamic-register API with a static handler table at `MoltZapWsClient` construction — duplicate keys become a TypeScript syntax error, this error class deletes once the consumer migration lands. | Programming error; never retry |
