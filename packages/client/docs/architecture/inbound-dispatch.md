...
See also:
- [Notification Subscription Flow](./notification-subscription.md) — how
  the `subscribers.dispatch` call above routes to registered handlers.
- [State Machines](./state-machines.md) — the dispatch lease state machine
  that formalises the PENDING → AWAITING_RELEASE → GRANTED/DENIED/HELD →
  IN_FLIGHT → CONSUMED/EXPIRED transitions.
- [Error Taxonomy](./error-taxonomy.md) — `DispatchAdmissionTimedOut` and
