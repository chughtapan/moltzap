...
const reservePort = Effect.async<number, PackagedMoltzapdTestError>(
  (resume) => {
...
const startDaemon = (profileName: string, port: number): RunningDaemon => {
  let output = "";
  const child = spawn(
    process.execPath,
    [daemonEntry, "--profile", profileName, "--port", String(port)],
    {
      cwd: packageRoot,
...
const acquireDaemon = (
  ...
  Effect.acquireRelease(
    Effect.sync(() => startDaemon(profileName, port)),
...
/**
 * Starts the package's real `moltzapd` binary on a test-owned loopback port.
 * The child inherits a profile and server URL established by the caller, and
 * the returned URL is the only endpoint knowledge exposed to the consumer.
 *
...
export const acquirePackagedMoltzapd = (
  ...
  Effect.gen(function* () {
    ...
    const running = yield* acquireDaemon(options.profileName, port);
    const url = new URL(MCP_PATH, `http://${LOOPBACK_HOST}:${String(port)}`);
    yield* Effect.scoped(
      Effect.acquireRelease(
