...
// Canary #1b element pin — the `extends … <def, any>` check above uses
// `any`, which accepts ANY refined `R` and therefore pins nothing about
// the element the overload actually produces. This canary holds an EXACT
// `Equal<>` assertion against the resolved Stream element type so the
// `any` escape hatch cannot mask element drift.
//
