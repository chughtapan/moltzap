...
/**
 * Escape MDX-significant characters in CLI help prose. A bare angle-bracket
 * placeholder (e.g. an `&lt;conversationId>` token) or `{...}` in a description
 * parses as a JSX tag / expression on the Mintlify page; escape both to their
 * HTML entities outside inline-code backtick spans (where MDX already treats
 * the content literally).
 */
const escapeMdxProse = (text: string): string => {
  let inFence = false;
  return text
    .split("\n")
    .map((line) => {
