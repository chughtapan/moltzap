...
│   ├── method.ts              # defineRpc, defineNotification, RpcDefinition (slotDisposition + capabilities optional)
...
│   ├── defaults.ts            # SlotDisposition, optionalForbidden, optionalNoOp (per-slot fail-CLOSED defaults)
...
| `optionalForbidden` / `optionalNoOp` | transport/defaults | Per-slot fail-CLOSED default markers |
