 ...
 *   3. the app/agent callable groups keep literal tags instead of widening to
 *      the whole string universe.
 *
...
type MemberWithTag<G, Name extends string> =
  MemberOf<G> extends infer R
    ? R extends Rpc.Rpc<Name, infer _P, infer _S, infer _E>
      ? R
      : never
    : never;
...
type _N3 = Expect<Equal<IsNever<MemberOf<typeof AgentCallableGroup>>, false>>;
type _N4 = Expect<Equal<IsNever<MemberOf<typeof AppCallableGroup>>, false>>;

type NotificationDescriptorTags =
  AnyNotificationDefinition["notificationRpc"]["_tag"];
type ReverseTags = MemberOf<typeof ReverseRpcGroup>["_tag"];
type _N5 = Expect<
...
type _AgentNotWidened = Expect<
  Equal<AgentCallableTags, string> extends true ? false : true
>;
type _AppNotWidened = Expect<
  Equal<AppCallableTags, string> extends true ? false : true
>;
