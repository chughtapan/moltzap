...
import { TaskAuthorizeDispatch } from "./apps.js";
import {
  taskCallbackRpcMethods,
  type TaskCallbackRpcMethodName,
...
describe("admission RPC registration", () => {
  it("registers the surviving admission verb under taskCallback", () => {
    const taskCallbackNames = taskCallbackRpcMethods.map((m) => m.name);
    expect(taskCallbackNames).toEqual([
      TaskAuthorizeDispatch.name,
    ] satisfies TaskCallbackRpcMethodName[]);
  });
});
