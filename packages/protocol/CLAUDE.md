...
- `src/task/` — Task layer: `Task*` + `TaskConversation*` + `Messages*` RPC descriptors and notification definitions. `AppId` brand, `DEFAULT_APP_ID` constant, `ParticipantNotAdmittedError`. Per-flow walkthrough: `docs/architecture/task-conversation-family.md`. Type canaries: `src/task/task-conversation-family.types-check.ts`.
