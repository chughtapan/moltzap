...
- [WebSocket connection lifecycle](./ws-connection-lifecycle.md) — where `leaseRegistry.abandon` is called in the disconnect finalizer
- [Server-initiated callback](./server-initiated-callback.md) — moderator round-trip that produces the verdict
- [AppHost hook unification](./app-host-hook-unification.md) — how verdicts are shaped by `wrapHookEffectWithEnvelope`
