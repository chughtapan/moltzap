...
**Capability auto-provision** is wired but unused: the dispatcher reads
`definition.capabilities` per frame and threads
`Effect.provideServiceEffect` from a `CapabilityProviderTable`. Today
every `defineRpc` ships with `capabilities` absent (effective
`readonly []`), so the codepath threads zero tags. The handler-side
`requireTmAuthority` pattern in
`packages/server/src/task/services/task.service.ts` is the live authority
check until the capability tags are populated.
...
(see [server/04](./04-server-initiated-callback.md)). On the client side
...
- [§04 Server-initiated callback](./04-server-initiated-callback.md) — `AppHost.runAuthorizeDispatch` fork + verdict routing + `leaseRegistry.resolve`
