...
import { NetworkLayerScope } from "../layer-scope.js";
...
>(
  ...
  def: {
    ...
    ) => Effect.Effect<Static<R>, RpcFailure, ConnIdTag | NetworkLayerScope>;
  ...
  return defineMethod(definition, {
    handler: (params, ctx) =>
      def
        .handler(params, ctx)
        .pipe(Effect.provideService(NetworkLayerScope, NETWORK_SCOPE)),
