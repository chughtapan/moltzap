...
function startServerEffect(
  ...
  return Effect.gen(function* () {
    ...
    yield* registerConfiguredApps(app, bootPlan);
...
function registerConfiguredApps(
  ...
  if (bootPlan.apps.length === 0) return Effect.void;
  return Effect.forEach(
    bootPlan.apps,
    (appRef) => registerConfiguredApp(app, bootPlan.configDirectory, appRef),
...
function registerConfiguredApp(
  ...
  appRef: { readonly manifest: string },
): Effect.Effect<void> {
  return Effect.gen(function* () {
    const manifestPath = resolveManifestPath(configDirectory, appRef.manifest);
