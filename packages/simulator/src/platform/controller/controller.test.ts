...
import { SocietyPlatform } from "../platform.js";
...
function operations(
  imported: unknown,
  execution: ReturnType<ControllerOperations["executeRunSpec"]>,
...
test("imports and executes the single named runSpec exactly once", () =>
  Effect.gen(function* () {
    ...
    const result = yield* runControllerWith(VALID_ENVIRONMENT, {
...
test("exports completed ledger bytes with the completion marker last", () =>
  Effect.gen(function* () {
    ...
    yield* exportCompletedLedgerWith(
      {
...
test("retains a completed receipt before returning the controller summary", () =>
  Effect.gen(function* () {
    const calls: string[] = [];
    let exported: ControllerLedgerExportInput | undefined;
    const result = yield* runControllerWith(GKE_ENVIRONMENT, {
...
test("reports a retained-artifact export failure before controller exit", () =>
  Effect.gen(function* () {
    ...
    const observed = yield* runControllerWith(GKE_ENVIRONMENT, {
      importModule: () => Promise.resolve({ runSpec }),
      executeRunSpec: () =>
        Effect.succeed(
          new ProgramFinished({
...
test("rejects any additional module export before execution", () =>
  Effect.gen(function* () {
    ...
    const failure = yield* runControllerWith(
      VALID_ENVIRONMENT,
      operations({ runSpec, default: runSpec }, execution),
    ).pipe(Effect.flip);

    assert.instanceOf(failure, ControllerFailure);
...
test("sanitizes module and execution failures", () =>
  Effect.gen(function* () {
    ...
    const moduleFailure = yield* runControllerWith(VALID_ENVIRONMENT, {
    ...
    const executionFailure = yield* runControllerWith(
      VALID_ENVIRONMENT,
...
test("treats a RunInfrastructureFailed outcome as controller failure", () =>
  Effect.gen(function* () {
    const infrastructureSecret = "ledger-mount-secret-detail";
    const outcome = new RunInfrastructureFailed<
      Readonly<Record<string, never>>
    >({
      cause: Cause.fail(
        new SimulatorInfrastructureFailure({
          detail: infrastructureSecret,
        }),
      ),
      receipt: IncompleteLedgerReceipt.make({ ledger: LEDGER_REFERENCE }),
    });
    const observed = yield* runControllerWith(
      VALID_ENVIRONMENT,
    ...
    assert.notInclude(observed.message, infrastructureSecret);
  }));
