...
/** Native boundaries replaced by deterministic GKE entry-point tests. */
export interface GkeRunOperations {
  readonly readProfile: () => Effect.Effect<string, unknown>;
  readonly runSociety: (
    args: readonly string[],
    environment: LocalRunEnvironment,
    profile: GkeKubernetesExecutionProfile,
  ) => Effect.Effect<LocalRunResult, LocalRunFailed>;
}

function configurationFailure(detail: string): LocalRunFailed {
  return new LocalRunFailed({ stage: "configuration", detail });
}

function required(environment: LocalRunEnvironment, key: string): string {
...
function checkedArtifactBucket(environment: LocalRunEnvironment): string {
...
const liveOperations: GkeRunOperations = Object.freeze({
  readProfile: () =>
    FileSystem.FileSystem.pipe(
      Effect.flatMap((fileSystem) => fileSystem.readFileString(PROFILE_PATH)),
      Effect.provide(NodeContext.layer),
    ),
  runSociety: runKubernetesSociety,
});
 ...
 */
export function runGkeSocietyWith(
  args: readonly string[],
  environment: LocalRunEnvironment,
  operations: GkeRunOperations,
): Effect.Effect<LocalRunResult, LocalRunFailed> {
  return operations.readProfile().pipe(
    Effect.mapError(() =>
    ...
    Effect.flatMap((profile) =>
      operations.runSociety(args, environment, profile),
    ),
    Effect.withSpan("runGkeSocietyWith"),
  );
}

/**
 * Run one operator-selected experiment with the checked-in GKE profile.
 * @param args One `.mjs` RunSpec entrypoint.
 * @param environment GKE identities plus shared image and Temporal settings.
 * @returns The coarse run result and ephemeral run identity.
 */
function runGkeSociety(
  args: readonly string[],
  environment: LocalRunEnvironment,
): Effect.Effect<LocalRunResult, LocalRunFailed> {
  return runGkeSocietyWith(args, environment, liveOperations);
}
