...
function fakeClusterLayer(
  ...
  return Layer.merge(
    fakeCluster(resolvedStorage, resolvedRouter),
    Layer.succeed(Cluster, cluster),
  );
}
...
function cohortGateCase() {
  return Effect.scoped(
    Effect.gen(function* () {
      ...
      const clusterReleased = yield* Ref.make(false);
      ...
      const cluster = makeFakeCluster({
        ...
        onRelease: Ref.set(clusterReleased, true),
      });
      const spec = RunSpec.define({
        id: "acme.run-spec-cohort/v1",
        events: [customerEvents],
        agents: { alice, bob },
        cluster: fakeClusterLayer(cluster, storage),
      ...
      assert.isTrue(yield* Ref.get(clusterReleased));
      yield* assertCohortLedger(storage);
    }),
  );
}

// @agent-code-guard/regression-only: deterministic cluster gates prove exact dispatch, failure, evidence, and cleanup ordering
test(
  "Run.execute waits for the complete cluster cohort and cleans up",
  cohortGateCase,
);
...
test("Run.execute never dispatches an incomplete roster", () =>
  Effect.gen(function* () {
    ...
    const spec = RunSpec.define({
      ...
      events: [],
      agents: { primary, peer },
      cluster: fakeClusterLayer(cluster),
      execute: () =>
        Ref.update(executions, (count) => count + 1).pipe(
    ...
    if (result instanceof ClusterLost) {
      assert.isTrue(
        Array.from(Cause.failures(result.cause)).some(
          (cause) => cause instanceof ClusterError,
        ),
      );
    }
    assert.strictEqual(yield* Ref.get(executions), 0);
...
test("Run.execute fails on post-dispatch cluster loss without replay", () =>
  Effect.gen(function* () {
    const programStarted = yield* Deferred.make<undefined>();
    const platformLost = yield* Deferred.make<never, ClusterError>();
    const executions = yield* Ref.make(0);
    const runtimeReleased = yield* Ref.make(false);
    const clusterReleased = yield* Ref.make(false);
    const runtime = defineFakeRuntime({
      name: "run-spec-cluster-loss",
      configuration: configuration("run-spec-cluster-loss"),
      acquire: () =>
        Effect.acquireRelease(
          Effect.succeed({ gateway: undefined, termination: Effect.never }),
          () => Ref.set(runtimeReleased, true),
    ...
    const spec = RunSpec.define({
      id: "acme.run-spec-cluster-loss/v1",
      events: [],
      agents: { alice: runtime },
      cluster: fakeClusterLayer(cluster),
      execute: () =>
    ...
    if (result instanceof ClusterLost) {
      ...
      assert.isTrue(
        Array.from(Cause.failures(result.cause)).some(
          (cause) =>
            cause instanceof ClusterError && cause.detail === failure.detail,
        ),
      );
    }
    assert.strictEqual(yield* Ref.get(executions), 1);
