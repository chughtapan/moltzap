...
type DeliveryMailbox = Mailbox.Mailbox<ReceivedMessage, NetworkError>;
...
interface InboxRuntime {
  readonly all: PubSub.PubSub<Take.Take<ReceivedMessage, NetworkError>>;
...
): Stream.Stream<ReceivedMessage, NetworkError> {
  ...
  return runtime.transition.withPermits(1)(
    Effect.gen(function* () {
      ...
      if (conversation === undefined) {
        conversation = yield* Mailbox.make<ReceivedMessage, NetworkError>();
  ...
  return (conversationId) =>
    runtime.transition.withPermits(1)(
      Effect.gen(function* () {
        ...
        const mailbox = yield* Mailbox.make<ReceivedMessage, NetworkError>();
        if (state.exit !== undefined) {
...
function runIngress<Name extends string>(
  ...
  runtime: InboxRuntime,
  received: Stream.Stream<ReceivedMessage, NetworkError>,
) {
...
function makeInbox<Name extends string>(
  ...
  return Effect.gen(function* () {
    const runtime: InboxRuntime = {
      all: yield* PubSub.unbounded<Take.Take<ReceivedMessage, NetworkError>>(),
