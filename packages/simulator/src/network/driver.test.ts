...
interface Harness {
  readonly acquire: RouterDriverAcquirer;
...
function harness(): Harness {
  ...
  const acquire: RouterDriverAcquirer = () =>
    Effect.gen(function* () {
      yield* Effect.addFinalizer(() =>
        Effect.sync(() => {
...
function providerFor(acquireDriver: RouterDriverAcquirer) {
  return RouterProvider.pipe(
    Effect.provide(
      routerProviderLayer({ startupTimeout: STARTUP_TIMEOUT }).pipe(
        Layer.provide(Layer.succeed(RouterOperations, acquireDriver)),
      ),
    ),
  );
}
...
describe("MoltZap router", () => {
  it("keeps identities stable and completes stopped after scoped release", () =>
    Effect.gen(function* () {
      const test = harness();
      const provider = yield* providerFor(test.acquire);
  ...
  it("maps acquisition and registration failures to network operations", () =>
    Effect.gen(function* () {
      const scope = yield* Scope.make();
      const unavailable = yield* providerFor(() =>
      ...
      const test = harness();
      const registrationFailed: RouterDriverAcquirer = (options) =>
      ...
      const provider = yield* providerFor(registrationFailed);
      const router = yield* provider.acquire.pipe(Scope.extend(scope));
  ...
  it("normalizes endpoint attachment and release-time collection failures", () =>
    Effect.gen(function* () {
      const base = harness();
      const acquire: RouterDriverAcquirer = (options) =>
        base.acquire(options).pipe(
          Effect.map((driver) => ({
            ...driver,
            attachEndpoint: () => Effect.fail("socket authentication failed"),
            stopAndCollect: Effect.fail(
              networkError("stop-router", "traffic collection failed"),
            ),
          })),
        );
      const provider = yield* providerFor(acquire);
      const scope = yield* Scope.make();
      const router = yield* provider.acquire.pipe(Scope.extend(scope));
