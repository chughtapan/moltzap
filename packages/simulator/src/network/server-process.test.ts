...
import { messageDatabasePathForVolume } from "./message-store.js";
import { routerSequence, type EndpointTransport } from "./router.js";
import {
  makeServerProcessRouterProviderWith,
...
describe("controller MoltZap server process", () => {
  it("uses loopback inside the controller while advertising the Service to agents", () =>
    Effect.gen(function* () {
      ...
      const router = yield* provider(harness).acquire.pipe(Scope.extend(scope));
  ...
  it("stops the child and removes its data when readiness fails", () =>
    Effect.gen(function* () {
      ...
      const failure = yield* Effect.scoped(provider(harness).acquire).pipe(
        Effect.flip,
      );
