...
import { resolveInstalledPackageBin } from "../runtime/packages.js";
import {
...
function endpointMessages(
  ...
  return client
    ...
    .pipe(
      Effect.map((received) =>
        received.pipe(
          Stream.mapError((cause) => networkFailure("receive", cause)),
        ),
      ),
    );
}
...
function openConversationWith(
  ...
  return (participants: ParticipantIds) =>
    client
      ...
      .pipe(
        Effect.mapError((cause) => networkFailure("open-conversation", cause)),
...
function collectStoppedRouter<ProcessHandle>(
  ...
  owned: OwnedResources<ProcessHandle>,
  permit: Effect.Semaphore,
): Effect.Effect<RouterStopped, ReturnType<typeof networkFailure>> {
  return Effect.gen(function* () {
...
function acquireServerProcessDriver<ProcessHandle>(
  ...
  operations: ServerProcessRouterOperations<ProcessHandle>,
): Effect.Effect<MoltZapRouterDriver, unknown, Scope.Scope> {
  return Effect.uninterruptibleMask((restore) =>
    Effect.gen(function* () {
...
/**
 * Build the package-private router provider over explicit lifecycle operations.
 * @param options Options that control the operation.
 * @param operations Injectable lifecycle operations.
 * @internal
 * @returns The controller router provider.
 */
export function makeServerProcessRouterProviderWith<ProcessHandle>(
  options: ServerProcessRouterOptions,
  operations: ServerProcessRouterOperations<ProcessHandle>,
): RouterProviderService {
  return makeMoltZapRouterProviderWith(
...
/**
 * Build the package-private controller provider for an installed server process.
 * @param options Options that control the operation.
 * @internal
 * @returns The controller router provider.
 */
export function makeServerProcessRouterProvider(
  options: ServerProcessRouterOptions,
): RouterProviderService {
  return makeServerProcessRouterProviderWith(
    options,
