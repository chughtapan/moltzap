/** @file Atomic filesystem allocation, append, completion, digest, and artifact reads. */
 ...
 * @param root Directory that contains the ledger artifacts.
 ...
 * @param active Ledger whose durable record stream and phase advance together.
 * @param record Canonical JSON record without an NDJSON line terminator.
 ...
 * @param active Ledger whose completion phase is being committed.
 * @param candidate Canonical completion value and its exact encoded bytes.
...
function canonicalComposite(
  ...
  const fields = Object.entries(value)
    ...
    .map(([key, entry]) => `${quote(key)}:${encodeValue(entry)}`);
  return `{${fields.join(",")}}`;
}
