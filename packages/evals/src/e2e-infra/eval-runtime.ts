    ...
    return {
      ...
      teardown(): Effect.Effect<void, never, never> {
        return Effect.gen(function* () {
          ...
          yield* Effect.sync(() =>
            emitFleetStoppedIfStarted(input.runtime, input.agentName, startedRef),
          );
        });
      },
      getLogs(offset: number): LogSlice {
...
> {
  return Effect.gen(function* () {
    ...
    return {
      name: input.agentName,
      waitUntilReady(_timeoutMs: number): Effect.Effect<ReadyOutcome, never, never> {
        return Effect.sync(() => {
          emitFleetStartedOnce(
            input.runtime,
            input.agentName,
      ...
      teardown(): Effect.Effect<void, never, never> {
        return Effect.gen(function* () {
          ...
          yield* dockerManager.stopAgent(container).pipe(
            Effect.catchAll((err) =>
              Effect.sync(() =>
                logger.warn(
