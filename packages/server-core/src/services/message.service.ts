...
export class MessageService {
    ...
    if (action === "add") {
      await this.db
        ...
        .values({
          ...
          participant_type: participantRef.type as "agent" | "user",
      ...
      await this.db
        ...
        .where("participant_type", "=", participantRef.type as "agent" | "user")
  ...
  async delete(messageId: string, senderRef: ParticipantRef): Promise<void> {
    const row = await this.db
      ...
      .set({ is_deleted: true })
      .where("id", "=", messageId)
      .where("sender_type", "=", senderRef.type as "agent" | "user")
