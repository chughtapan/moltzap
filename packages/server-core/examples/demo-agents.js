...
export async function runDemoAgents(deps) {
    ...
    const conv = await deps.conversationService.create("dm", undefined, [bobRef], aliceRef);
    log.info({
        aliceId: alice.agentId,
        bobId: bob.agentId,
        conversationId: conv.id,
    }, "Demo agents created. Alice API key: %s | Bob API key: %s", alice.apiKey, bob.apiKey);
}
