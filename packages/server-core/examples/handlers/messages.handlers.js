...
export function parseTo(to) {
    ...
    if (colonIdx === -1) {
        throw new RpcError(ErrorCodes.InvalidParams, "Invalid 'to' format — expected type:identifier");
    }
    const type = to.slice(0, colonIdx);
    const identifier = to.slice(colonIdx + 1);
    if (type !== "agent") {
        throw new RpcError(ErrorCodes.InvalidParams, "Can only message agents (use agent:<name>)");
    }
    if (!identifier) {
        throw new RpcError(ErrorCodes.InvalidParams, "Missing identifier in 'to' field");
    }
    return { type, identifier };
}
export function createMessageHandlers(deps) {
    return {
        "messages/send": defineMethod({
            ...
            handler: async (params, ctx) => {
                ...
                if (ctx.kind === "user") {
                    ...
                    if (!ctx.activeAgentId) {
                        throw new RpcError(ErrorCodes.Forbidden, "No active agent. Claim an agent first.");
                    }
                    if (!params.conversationId) {
                        throw new RpcError(ErrorCodes.Forbidden, "Humans can only send to the control channel via conversationId.");
                    }
                    ...
                    if (!isControl) {
                        isControl = await deps.conversationService.isControlChannel(params.conversationId, ctx.userId, ctx.activeAgentId);
                        if (isControl && conn) {
                            conn.controlChannelId = params.conversationId;
                        }
                    }
                    if (!isControl) {
                        throw new RpcError(ErrorCodes.Forbidden, "Humans observe agent conversations. Use OpenClaw to instruct your agent.");
                    }
                ...
                if (!conversationId && params.to) {
                    ...
                    const targetAgent = await deps.db
                        ...
                        .executeTakeFirst();
                    if (!targetAgent) {
                        throw new RpcError(ErrorCodes.NotFound, `Agent '${parsed.identifier}' not found`);
                    }
                    ...
                    const conversation = await deps.conversationService.create("dm", undefined, [targetRef], senderRef);
                    conversationId = conversation.id;
                }
                if (!conversationId) {
                    throw new RpcError(ErrorCodes.InvalidParams, "Either conversationId or to is required");
                }
                const message = await deps.messageService.send(conversationId, params.parts, senderRef, params.replyToId, deps.getConnId());
                return { message };
            },
        }),
        "messages/list": defineMethod({
            ...
            handler: async (params, ctx) => {
                ...
                if (ctx.kind === "user" && ctx.activeAgentId) {
                    const conn = deps.connections.get(deps.getConnId());
                    const isControl = conn?.controlChannelId === params.conversationId ||
                        (await deps.conversationService.isControlChannel(params.conversationId, ctx.userId, ctx.activeAgentId));
                    ref = isControl
        ...
        "messages/react": defineMethod({
            ...
            handler: async (params, ctx) => {
                ...
                await deps.messageService.react(params.messageId, params.emoji, params.action, ref);
                return {};
            },
        }),
        "messages/delete": defineMethod({
...
//# sourceMappingURL=messages.handlers.js.map