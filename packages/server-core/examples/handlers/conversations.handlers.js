import { validators, EventNames, ErrorCodes, eventFrame, } from "@moltzap/protocol";
    ...
    return {
        "conversations/create": defineMethod({
            ...
            handler: async (params, ctx) => {
                ...
                if (ctx.kind === "user") {
                    ...
                    const isControlCreate = params.type === "dm" &&
                ...
                const conversation = await deps.conversationService.create(params.type, params.name, params.participants, creatorRef);
                ...
                for (const ref of params.participants) {
                    ...
                    for (const conn of deps.connections.getByParticipant(ref.type, ref.id)) {
                        conn.conversationIds.add(conversation.id);
                    }
                    deps.broadcaster.sendToParticipant(ref.type, ref.id, eventFrame(EventNames.ConversationCreated, { conversation }));
                }
                return { conversation };
            },
            requiresActive: true,
        ...
        "conversations/update": defineMethod({
            ...
            handler: async (params, ctx) => {
                ...
                const ref = ParticipantService.refFromContext(ctx);
                const conversation = await deps.conversationService.update(params.conversationId, params.name, ref);
                deps.broadcaster.broadcastToConversation(params.conversationId, eventFrame(EventNames.ConversationUpdated, { conversation }));
                return { conversation };
            },
            requiresActive: true,
        }),
        ...
        "conversations/mute": defineMethod({
            ...
            handler: async (params, ctx) => {
                ...
                const ref = ParticipantService.refFromContext(ctx);
                await deps.conversationService.mute(params.conversationId, ref, params.until);
        ...
        "conversations/addParticipant": defineMethod({
            ...
            handler: async (params, ctx) => {
                ...
                const ref = ParticipantService.refFromContext(ctx);
                const participant = await deps.conversationService.addParticipant(params.conversationId, params.participant, ref);
                // Subscribe the new participant's connections to the conversation
                for (const conn of deps.connections.getByParticipant(params.participant.type, params.participant.id)) {
        ...
        "conversations/removeParticipant": defineMethod({
            ...
            handler: async (params, ctx) => {
                ...
                const ref = ParticipantService.refFromContext(ctx);
                await deps.conversationService.removeParticipant(params.conversationId, params.participant, ref);
                return {};
            },
            requiresActive: true,
        }),
