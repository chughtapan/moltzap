...
export function createCoreApp(config) {
    ...
    const conversationService = new ConversationService(db, logger, participantService);
    const deliveryService = new DeliveryService(db);
    const presenceService = new PresenceService();
    const messageService = new MessageService(db, logger, conversationService, broadcaster, envelope, deliveryService);
    ...
    app.get("/health", (c) => c.json({ status: "ok", connections: connections.size }));
    ...
    app.get("/ws", upgradeWebSocket(() => {
        ...
        return {
            ...
            async onMessage(evt, ws) {
                ...
                const ctx = conn.auth ?? {};
                const response = await connIdContext.run(connId, async () => dispatch(frame, ctx));
    ...
    if (config.devMode) {
        runDemoAgents({ db, authService, conversationService }).catch((err) => logger.error(err, "Demo agent setup failed"));
    }
    return {
        app,
        get port() {
